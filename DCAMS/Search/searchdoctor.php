...
<div class="ltable">
									...
									<input type="submit" name="submit" value="SEARCH by doctor name"/></form>
									</div>
...
$result = mysqli_query($con, "SELECT * FROM doctorsabvailability
    WHERE doctorName LIKE '%{$name}%'");
	if ($result->num_rows > 0) {
    echo "<table align=center><tr><th> ID</th><th>Doctor Name</th><th>Day</th><th>Date</th><th>Time</th></tr>";
    // output data of each row

while ($row = mysqli_fetch_array($result))
{
    echo "<tr><td>" .$row["id"]."</td><td>".$row["doctorName"]."</td><td>".$row["Day"]."</td><td>".$row["Date"]."</td><td>".$row["Time"]."</td></tr>";
}
 echo "</table>";
}
else {
